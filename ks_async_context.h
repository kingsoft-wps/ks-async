...
public:
	...
	KS_ASYNC_INLINE_API _NO_INLINE ks_async_context& operator=(const ks_async_context& r) noexcept {
		if (this != &r) {
	...
	KS_ASYNC_INLINE_API _NO_INLINE ks_async_context& operator=(ks_async_context&& r) noexcept {
		if (this != &r) {
			__do_release_fat_data(m_fat_data_p);
			m_fat_data_p = r.m_fat_data_p;
			m_priority = r.m_priority;
...
private:
	...
	_NO_INLINE void do_bind_owner(SMART_PTR&& owner_ptr, std::false_type owner_ptr_is_weak) {
	...
	template <class SMART_PTR>
	_NO_INLINE void do_bind_owner(SMART_PTR&& owner_ptr, std::true_type owner_ptr_is_weak) {
...
public:
	KS_ASYNC_INLINE_API _NO_INLINE void swap(ks_async_context& r) noexcept {
		//if (this != &r) {
...
private:
	...
	_NO_INLINE void __do_release_fat_data(_FAT_DATA* fata_data_p) noexcept {
		if (fata_data_p != nullptr) {
