...
public:
	...
	explicit ks_async_task(ks_apartment* apartment, std::function<T(const ARGs&...)>&& fn, const ks_async_context& context = {}) {
		this->do_init(apartment, context, std::move(fn));
	}
	explicit ks_async_task(ks_apartment* apartment, const ks_async_context& context, std::function<T(const ARGs&...)>&& fn) { //only for compat
		this->do_init(apartment, context, std::move(fn));
	}
