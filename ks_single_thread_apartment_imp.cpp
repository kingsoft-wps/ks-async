...
bool ks_single_thread_apartment_imp::__run_nested_pump_loop_for_extern_waiting(void* object, std::function<bool()>&& extern_pred_fn) {
	auto d = m_d;
...
void ks_single_thread_apartment_imp::__awaken_nested_pump_loop_for_extern_waiting_once(void* object) {
	std::unique_lock<ks_mutex> lock(m_d->mutex);
	m_d->any_fn_queue_cv.notify_all();
}
