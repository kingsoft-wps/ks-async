...
public:
	...
	_NO_INLINE ks_any& operator=(const ks_any& r) noexcept {
		if (this != &r) {
	...
	_NO_INLINE ks_any& operator=(ks_any&& r) noexcept {
		if (this != &r) {
			__do_release_data(m_data_p);
			m_data_p = r.m_data_p;
			m_embed_tiny_trivial_x_mem = r.m_embed_tiny_trivial_x_mem;
...
private:
	...
	_NO_INLINE explicit ks_any(T*, X&& x, std::bool_constant<false>) {
		using XT = std::remove_cvref_t<T>;
	...
	template <class T, class X>
	_NO_INLINE explicit ks_any(T*, X&& x, std::bool_constant<true>) {
		using XT = std::remove_cvref_t<T>;
...
private:
	...
	_NO_INLINE static void __do_release_data(_DATA_HEADER* data_p) noexcept {
		if (data_p != nullptr && data_p != (void*)(-1)) {
