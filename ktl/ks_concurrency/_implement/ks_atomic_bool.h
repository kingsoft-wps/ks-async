...
#ifndef __KS_ATOMIC_BOOL_DEF
#define __KS_ATOMIC_BOOL_DEF
...
template <class T>
class ks_atomic_bool : private std::atomic<T> {
    static_assert(
        std::is_same<T, bool>::value,
        "ks_atomic_bool can only be used with bool types");

public:
    ks_atomic_bool() noexcept : std::atomic<T>(false) {} //与std不同，我们默认初始化为0
    ks_atomic_bool(T desired) noexcept : std::atomic<T>(desired) {}
    _DISABLE_COPY_CONSTRUCTOR(ks_atomic_bool);
