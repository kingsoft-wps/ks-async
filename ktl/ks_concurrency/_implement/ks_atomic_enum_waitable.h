...
#ifndef __KS_ATOMIC_ENUM_WAITABLE_DEF
#define __KS_ATOMIC_ENUM_WAITABLE_DEF
...
template <class T>
class ks_atomic_enum_waitable : private std::atomic<T> {
    static_assert(
        std::is_enum<T>::value && (sizeof(T) == 4 || sizeof(T) == 8),
        "ks_atomic_enum_waitable can only be used with 32-bit or 64-bit enum types");

public:
    ks_atomic_enum_waitable() noexcept : std::atomic<T>(T{}) {} //与std不同，我们默认初始化为0
    ks_atomic_enum_waitable(T desired) noexcept : std::atomic<T>(desired) {}
    _DISABLE_COPY_CONSTRUCTOR(ks_atomic_enum_waitable);
...
#endif // __KS_ATOMIC_ENUM_WAITABLE_DEF
