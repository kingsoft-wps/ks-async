...
template <class T>
class ks_atomic_integral_common : private std::atomic<T> {
    static_assert(
        std::is_integral<T>::value && !std::is_same<T, bool>::value,
        "ks_atomic_integral_common can only be used with integral (exclude bool) types");

public:
    ks_atomic_integral_common() noexcept : std::atomic<T>(T(0)) {} //与std不同，我们默认初始化为0
    ks_atomic_integral_common(T desired) noexcept : std::atomic<T>(desired) {}
    _DISABLE_COPY_CONSTRUCTOR(ks_atomic_integral_common);
...
#endif // __KS_ATOMIC_INTEGRAL_COMMON_DEF
