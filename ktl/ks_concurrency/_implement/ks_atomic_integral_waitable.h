...
#ifndef __KS_ATOMIC_INTEGRAL_WAITABLE_DEF
#define __KS_ATOMIC_INTEGRAL_WAITABLE_DEF
...
template <class T>
class ks_atomic_integral_waitable : private std::atomic<T> {
    static_assert(
        std::is_integral<T>::value && (sizeof(T) == 4 || sizeof(T) == 8),
        "ks_atomic_integral_waitable can only be used with 32-bit or 64-bit integral types");

public:
    ks_atomic_integral_waitable() noexcept : std::atomic<T>(T(0)) {} //与std不同，我们默认初始化为0
    ks_atomic_integral_waitable(T desired) noexcept : std::atomic<T>(desired) {}
    _DISABLE_COPY_CONSTRUCTOR(ks_atomic_integral_waitable);
...
#endif // __KS_ATOMIC_INTEGRAL_WAITABLE_DEF
