...
#ifndef __KS_ATOMIC_STORAGE_DEF
#define __KS_ATOMIC_STORAGE_DEF
...
template <class T>
class ks_atomic_storage : private std::atomic<T> {
public:
    ks_atomic_storage() noexcept : std::atomic<T>(T{}) {} //与std不同，我们默认初始化为0
    ks_atomic_storage(T desired) noexcept : std::atomic<T>(desired) {}
    _DISABLE_COPY_CONSTRUCTOR(ks_atomic_storage);
