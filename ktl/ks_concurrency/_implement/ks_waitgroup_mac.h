...
class ks_waitgroup_mac_ossync {
public:
    explicit ks_waitgroup_mac_ossync(ptrdiff_t desired)
        : m_counter(desired) {
        ASSERT(desired >= 0);
    ...
    _DISABLE_COPY_CONSTRUCTOR(ks_waitgroup_mac_ossync);
...
class ks_waitgroup_mac {
public:
    explicit ks_waitgroup_mac(ptrdiff_t desired) {
        if (m_use_ossync) {
            new (&m_ossync_impl) ks_waitgroup_mac_ossync(desired);
        } else {
            new (&m_gdc_impl) ks_waitgroup_mac_gdc(desired);
        }
    }

    ~ks_waitgroup_mac() {
        if (m_use_ossync) {
            m_ossync_impl.~ks_waitgroup_mac_ossync();
        } else {
            m_gdc_impl.~ks_waitgroup_mac_gdc();
        }
    }

    _DISABLE_COPY_CONSTRUCTOR(ks_waitgroup_mac);
...
private:
    union {
        ks_waitgroup_mac_ossync m_ossync_impl;
        ks_waitgroup_mac_gdc m_gdc_impl;
    };
