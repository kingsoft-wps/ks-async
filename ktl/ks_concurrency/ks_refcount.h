...
public:
    ...
    void add() {
        (void)m_atomic_value.fetch_add(1, std::memory_order_relaxed);
    }

    int sub_and_release() {
        int new_value = m_atomic_value.fetch_sub(1, std::memory_order_release) - 1;
        ASSERT(new_value >= 0);
        if (new_value == 0)
    ...
    _NODISCARD int peek_value(bool with_acquire = false) const {
        return m_atomic_value.load(with_acquire ? std::memory_order_acquire : std::memory_order_relaxed);
    }
