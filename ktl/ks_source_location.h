...
public:
    ...
    static ks_source_location __empty_inst() {
        return ks_source_location(nullptr, 0, nullptr); 
    }
...
inline 
ks_source_location __make_source_location(const char* file_name, unsigned int line, const char* function_name) {
    return ks_source_location(file_name, line, function_name);
}

#define current_source_location()  \
    (__make_source_location(__FILE__, __LINE__, __FUNCTION__))
